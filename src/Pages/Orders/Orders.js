...
const Orders = () => {
  ...
  return (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 w-11/12 mx-auto my-10 text-center">
      {/* Search */}
      <input type="text" placeholder="Search here" className="input input-bordered bg-slate-200  w-11/12 mx-auto" />
      ...
      <div><button disabled className='btn border-none bg-primary rounded-lg text-white font-semibold w-11/12 '>Download All Orders <AiOutlineCloudDownload className="text-2xl ml-2"/></button></div>
        ...
        <Posts products={currentPost} isLoading={isLoading}></Posts>
         ...
         <Pagination postPerPage = {postPerPage} totalPosts = {products.length} paginate = {paginate}  ></Pagination>
         </div>
